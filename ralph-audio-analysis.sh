#!/bin/bash
# Ralph: Autonomous loop for Audio Analysis & UI Polish (Sub-PRD #144, worktree: audio-analysis-ui)
# Issues: #148 (Transients), #149 (Snap), #150 (Slicing), #151 (Piano Roll), #152 (Info Pane), #158 (Search Everywhere), #159 (Shortcut Registry)
# Uses a SINGLE persistent worktree so commits accumulate across iterations.
# Usage: ./ralph-audio-analysis.sh <iterations>
set -e
...
if [ ! -f "$PROGRESS" ]; then
  ...
  echo "# Issues: #148-#152 (Transients, Snap, Slicing, Piano Roll, Info Pane)" >> "$PROGRESS"
...
- Sub-PRD file: tasks/prd-pro-daw-analysis.md (read this for full context)

## CRITICAL: Issue Scope
You may ONLY work on these specific issue numbers: #148, #149, #150, #151, #152, #158, #159.
...
- Transient detection: use Accelerate/vDSP for FFT, spectral flux algorithm
- Piano roll: replace VirtualKeyboardView with full-range indicator, use Canvas for efficient drawing
- Info pane: centralized registry, positioned at bottom of left sidebar
...
2. Run: gh issue list --repo $REPO --state open --json number,title,body --limit 50
   Filter to ONLY issues #148, #149, #150, #151, #152, or #158. Ignore all other issues.
3. For each issue, check its 'Blocked by' section. Skip blocked issues (where the blocking issue is still open).
4. Pick the lowest-numbered unblocked issue from the allowed set (#148, #149, #150, #151, #152, #158).
5. Read the full issue: gh issue view <number> --repo $REPO
6. Read the sub-PRD file: tasks/prd-pro-daw-analysis.md
7. Explore the codebase thoroughly before making changes. Key files:
   ...
   - Sources/LoopsApp/Views/MIDI/VirtualKeyboardView.swift
...
If no open issues from (#148, #149, #150, #151, #152, #158) remain, output <promise>COMPLETE</promise>.")
